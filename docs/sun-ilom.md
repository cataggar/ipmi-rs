...
packet. LED OEM requests target the selected ILOM at the current physical
generic-device SDR locator's LUN; the locator's access/slave addresses and
channel are **payload fields, not an IPMB command destination**. Both the
identity check and OEM packet target the same ILOM. No board-product or
model-specific capability is inferred.
...
| `sun_delete_ssh_key(Approved, uid)` / `sun_set_ssh_key(Approved, uid, public_key)` | 0x02 one UID byte / 0x01 `[uid,block index or FF for last,length,≤64 key bytes]`, empty success. UID 1..63, bounded single-line OpenSSH public key only, ≤16 KiB *transmitted*; an optional terminal newline is omitted. No local file access. |
...
| `sun_get_file(id, max_bytes)` / `sun_get_behavior(id)` | 0x44, subcommands 11/15. First check typed ILOM version ≥**3.2.0.0**; file request `[11,16-byte ID, big-endian u32 block]` → big-endian block/size, EOF and ≤1024 bytes, returns caller-bounded bytes (≤1 MiB). The local transport accommodates the 1,034-byte reply (completion + header + full block). Behavior request `[15,32-byte ID]` → typed boolean. No file saving/printing. |
